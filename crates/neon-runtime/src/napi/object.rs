...
pub unsafe extern "C" fn new(out: &mut Local, env: Env) {
    (super::NAPI.napi_create_object)(env, out as *mut _);
}
...
pub unsafe extern "C" fn get_own_property_names(out: &mut Local, env: Env, object: Local) -> bool {
    ...
    if (super::NAPI.napi_get_property_names)(env, object, raw_names.as_mut_ptr()) != napi::napi_status::napi_ok {
        return false;
    }
    // And a "fixed" array for the actual return value:
    let mut fixed_names = MaybeUninit::uninit();
    if (super::NAPI.napi_create_array)(env, fixed_names.as_mut_ptr()) != napi::napi_status::napi_ok {
        return false;
    }
    ...
    *out = fixed_names;
    ...
    for index in 0..raw_len {
        ...
        // a string.
        if (super::NAPI.napi_has_own_property)(env, object, property_name, &mut is_own_property as *mut _) != napi::napi_status::napi_ok {
            return false;
        }
...
pub unsafe extern "C" fn get_index(out: &mut Local, env: Env, object: Local, index: u32) -> bool {
    let status = (super::NAPI.napi_get_element)(env, object, index, out as *mut _);
...
pub unsafe extern "C" fn set_index(out: &mut bool, env: Env, object: Local, index: u32, val: Local) -> bool {
    let status = (super::NAPI.napi_set_element)(env, object, index, val);
...
pub unsafe extern "C" fn get_string(env: Env, out: &mut Local, object: Local, key: *const u8, len: i32) -> bool {
    ...
    if (super::NAPI.napi_create_string_utf8)(env, key as *const i8, len as usize, key_val.as_mut_ptr())
    ...
    if (super::NAPI.napi_get_property)(env, object, key_val.assume_init(), out as *mut _)
        != napi::napi_status::napi_ok
    {
        return false;
    }
...
pub unsafe extern "C" fn set_string(env: Env, out: &mut bool, object: Local, key: *const u8, len: i32, val: Local) -> bool {
    ...
    *out = true;
    ...
    if (super::NAPI.napi_set_property)(env, object, key_val.assume_init(), val)
        != napi::napi_status::napi_ok
    {
        *out = false;
        return false;
...
pub unsafe extern "C" fn get(out: &mut Local, env: Env, object: Local, key: Local) -> bool {
    let status = (super::NAPI.napi_get_property)(env, object, key, out as *mut _);
...
pub unsafe extern "C" fn set(out: &mut bool, env: Env, object: Local, key: Local, val: Local) -> bool {
    let status = (super::NAPI.napi_set_property)(env, object, key, val);
    *out = status == napi::napi_status::napi_ok;
