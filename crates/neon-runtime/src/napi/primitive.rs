...
pub unsafe extern "C" fn boolean(out: &mut Local, env: Env, b: bool) {
    (super::NAPI.napi_get_boolean)(env, b, out as *mut Local);
}
...
pub unsafe extern "C" fn boolean_value(env: Env, p: Local) -> bool {
    let mut value = false;
    assert_eq!((super::NAPI.napi_get_value_bool)(env, p, &mut value as *mut bool), napi::napi_status::napi_ok);
    value
}
...
pub unsafe extern "C" fn number(out: &mut Local, env: Env, v: f64) {
    (super::NAPI.napi_create_double)(env, v, out as *mut Local);
}
...
pub unsafe extern "C" fn number_value(env: Env, p: Local) -> f64 {
    let mut value = 0.0;
    assert_eq!((super::NAPI.napi_get_value_double)(env, p, &mut value as *mut f64), napi::napi_status::napi_ok);
    return value;
}
